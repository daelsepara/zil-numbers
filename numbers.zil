...
<INSERT-FILE "parser.zil">

<CONSTANT GAME-BANNER "numbers demo">

<GLOBAL INPUT 0>

<ROOM NUMBERS-ROOM
    (DESC "At the computer terminal")
    (FLAGS LIGHTBIT)>

<ROUTINE GO ()
    <V-VERSION>
    <CRLF>
    <SETG HERE ,NUMBERS-ROOM>
    <GOTO ,HERE>
    <NUMBERS-LOOP>>

<ROUTINE NUMBERS-LOOP ("AUX" SUM)
    <REPEAT ()
        <CRLF>
        <TELL "Enter numbers separated by a space or a comma , (enter nothing or 'quit' to exit):">
        <CRLF>
        <SETG SUM 0>
        <READLINE T>
        <SETG P-LEN <GETB ,LEXBUF 1>>
        <COND (<0? ,P-LEN> <QUIT>)>
        <COND (<=? <GETWORD? 1> ,W?QUIT> <QUIT>)>
        <DO (I 1 ,P-LEN)
            <SET SUM <+ .SUM <CONVERT-TO-NUMBER .I 10>>>
        >
        <CRLF>
        <TELL "Sum: " N .SUM>
        <CRLF>
